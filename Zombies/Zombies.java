...
public class Zombies implements Comparable<Zombies>{
    private int type, zombieDamage, lane, coorY, yp, health;
        ...
        timer=new Timer(interval*1000, new ActionListener(){
            public void actionPerformed(ActionEvent e) {
                if(n<max){
                    ...
                    Playing.zombies.add(new Zombies(setType())); //deploy zombie
    ...
    public void attack(){
        ...
        if(Plants.getOcc(lane, yp)!=0){ //intersect plant
            A: for(Plants plant: Playing.plants){
    ...
    public static void playAudio(){
        ...
        if(n==wave+1){
            ...
            Playing.setWave(2); //set wave to 2
        }else if(n==wave+2){
            Audio.brain1();
            Playing.setWave(0); //set wave to 0
        }else if(n==wave+5){
            Audio.groan3();
        }else if(n==wave+8){
            Audio.groan1();
